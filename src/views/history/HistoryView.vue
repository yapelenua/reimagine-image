<template>
  <div style="padding: 32px; min-height: 100%">
    ...
    <div class="flex items-center justify-between mb-8">
      <h1 style="font-size: 1.1rem; font-weight: 600; color: var(--foreground); margin: 0">
        History
      </h1>
    ...
    >
      <span
        ...
        style="font-size: 2rem; color: var(--destructive)"
      />
    ...
    >
      <span
        class="mdi mdi-image-off-outline"
        style="font-size: 2.5rem; color: var(--muted-foreground)"
      />
      <p style="font-size: 0.875rem; color: var(--muted-foreground); margin: 0">
        No saved images yet.
    ...
    >
      ...
      >
        ...
        <div style="padding: 8px 12px">
          <p class="truncate" style="font-size: 0.72rem; color: var(--muted-foreground); margin: 0">
            {{ item.preset?.fileName ?? item.id }}
          </p>
        </div>
      </div>
    </div>
...
onMounted(() => {
  if (authStore.user) store.fetchHistory()
})
watch(
  () => authStore.user,
  u => {
    if (u) store.fetchHistory()
  },
)
</script>
