...
export const useImageStore = defineStore('image', () => {
  ...
  async function saveToHistory(): Promise<string | null> {
    ...
    try {
      ...
      const imageBlob = await new Promise<Blob>(res =>
      ...
      const [imgUp, jsonUp] = await Promise.all([
        supabase.storage
          .from('images')
          .upload(`${base}/image.jpg`, imageBlob, { contentType: 'image/jpeg' }),
        supabase.storage
          .from('images')
          .upload(`${base}/preset.json`, presetBlob, { contentType: 'application/json' }),
      ])
