  ...
  <v-dialog :model-value="true" max-width="360" @update:model-value="$emit('close')">
    <div style="background: var(--card); border-radius: var(--radius); overflow: hidden; display: flex; flex-direction: column;">
      ...
      <div style="padding: 16px 20px; border-bottom: 1px solid var(--border); display: flex; align-items: center; justify-content: space-between;">
        <p style="font-size: 0.875rem; font-weight: 600; color: var(--foreground); margin: 0;">Save Preset</p>
      ...
      <div style="padding: 20px; display: flex; flex-direction: column; gap: 12px;">
        ...
        />
        <p v-if="error" style="font-size: 0.75rem; color: var(--destructive); margin: 0;">{{ error }}</p>
      </div>

      <div style="padding: 12px 20px; border-top: 1px solid var(--border); display: flex; justify-content: flex-end; gap: 8px;">
        <UiButton variant="secondary" size="sm" @click="$emit('close')">Cancel</UiButton>
        <UiButton size="sm" :loading="saving" :disabled="!name.trim()" @click="handleSave">Save</UiButton>
      </div>
